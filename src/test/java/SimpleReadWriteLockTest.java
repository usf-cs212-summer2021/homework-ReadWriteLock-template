...
public class SimpleReadWriteLockTest {
	...
	public class B_WriteLockTest {		
		...
		/**
		 * Tests the child lock behaves as expected for multiple lock calls. Since
		 * there is only one active writer (even though calling lock multiple times),
	...
	public class D_ActiveWriterTest {
		...
		public void testWriteThenRead() {
			...
			List<String> expected = List.of(
					"Write Lock", "Read Lock", "Read Lock", 
	...
	public class E_LockExceptionTest {
		/**
		 * Tests that calling unlock() without previously calling lock()
		...
		public void testWrongWriter() {
			...
			boolean result = !handler.thrown.isEmpty() && 
					handler.thrown.get(0).getClass() == ConcurrentModificationException.class;
	 ...
	 */
	public static void testOutput(Thread[] workers, List<String> actual, List<String> expected, long timeout) {
		JUnitThreadHandler handler = new JUnitThreadHandler();
		...
		if (handler.thrown.isEmpty()) {
			Assertions.assertEquals(expected, actual, () -> DEBUG.formatted(
					String.join("\n", expected), String.join("\n", actual)));
		}
		else {
			String debug = "Unexpected exception(s) occurred within run() method: " + handler.thrown;
	...
	public static class JUnitThreadHandler implements Thread.UncaughtExceptionHandler {
	...
	public static class MultipleLockWorker extends Thread {
		 ...
		 */
		public MultipleLockWorker(SimpleLock lock, List<String> output, String prefix, int repeats) {
			this.lock = lock;
			this.output = output;
			this.prefix = prefix;
